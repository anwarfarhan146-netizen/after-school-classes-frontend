    ...
    <nav class="navbar navbar-dark bg-primary">
      <div class="container">
        ...
        <div class="navbar-text d-flex align-items-center">
          <!-- Show different text based on current view -->
...
export default {
  ...
  computed: {
    ...
    filteredLessons() {
      let sortedLessons = [...this.searchedLessons];
    ...
    totalSpaces() {
      return this.filteredLessons.length * 5;
    },
    availableSpaces() {
      return this.filteredLessons.reduce((total, lesson) => total + lesson.space, 0);
    },
    totalItemsInCart() {
    ...
    showCartButton() {
      return this.totalItemsInCart > 0;
    },
    isCheckoutValid() {
  ...
  methods: {
    ...
    addToCart(lesson) {
      if (lesson.space > 0) {
        const existingCartItem = this.cart.find(item => item.id === lesson._id);
    ...
    validateName() {
      ...
      if (!this.checkoutForm.name.trim()) {
        this.checkoutForm.errors.name = 'Name is required';
      } else if (!nameRegex.test(this.checkoutForm.name)) {
        this.checkoutForm.errors.name = 'Name must contain only letters and spaces';
      } else {
        this.checkoutForm.errors.name = '';
      }
    },
    ...
    validatePhone() {
      ...
      if (!this.checkoutForm.phone.trim()) {
        this.checkoutForm.errors.phone = 'Phone number is required';
      } else if (!phoneRegex.test(this.checkoutForm.phone)) {
        this.checkoutForm.errors.phone = 'Phone number must contain only numbers';
      } else {
        this.checkoutForm.errors.phone = '';
      }
    },
